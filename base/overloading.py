...
#     import typing
# except ImportError:
typing = None  # now problems with typing
...
def overload(func):
    ...
    fn = unwrap(func)
    ensure_function(fn)
    f_name = get_full_name(fn)
    if f_name.find('<locals>') >= 0:
        raise OverloadingError("The 'overload' syntax cannot be used with nested functions. "
                               "Decorators must use functools.wraps().")
    try:
        return register(__registry[f_name], func)
    except KeyError:
        __registry[f_name] = overloaded(func)
        return __registry[f_name]
...
def register(dispatcher, func, *, hook=None):
    ...
    if hook:
        ...
        for i, type_ in enumerate(signature.types):
            if not isinstance(type_, type) and False:  # !!!
...
def get_signature(func):
    ...
    type_hints = func.__annotations__  # typing.get_type_hints(func) if typing else func.__annotations__
...
def normalize_type(type_, level=0):
    ...
    """
    if not typing or type_ is AnyType:  # or not isinstance(type_, typing.TypingMeta)
        return type_
...
def ensure_function(func):
    if not isinstance(func, FunctionType) and False:  # !!!
